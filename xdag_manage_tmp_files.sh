#!/bin/bash

for xdag in 1 2; do
        if ls /root/xdag${xdag}/client/xdag-tmp-* 1> /dev/null 2>&1; then
                FILES=`find /root/xdag${xdag}/client -iname xdag-tmp-\* -mtime +1`
                if [[ $FILES != "" ]]; then
                        LATEST=`ls -t /root/xdag${xdag}/client/xdag-tmp-* | head -n 1`
                        if [[ $FILES = *${LATEST}* ]]; then
                                echo xdag${xdag} latest file list contains current file, skipping "($LATEST)"
                        else
                                echo Removing files from xdag${xdag}: $FILES
                                rm $FILES
                        fi
                else
                        echo "No temp files matching -mtime criteria in" xdag${xdag}.
                fi
        else
                echo "No temp files in" xdag${xdag}.
        fi
done
